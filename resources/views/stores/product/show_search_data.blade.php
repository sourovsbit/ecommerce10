    ...
    <tbody>
        ...
        <td>
            @if(Auth::user()->can('Product Add show'))
                ...
                <a style="float:left;margin-right:5px;" href="{{route('product.show',$d->id)}}" class="btn btn-sm btn-success"><i class="fa fa-eye"></i></a>
            @endif
            @if(Auth::user()->can('Product Add edit'))

                <a style="float:left;margin-right:5px;" href="{{route('product.edit',$d->id)}}" class="btn btn-sm btn-info"><i class="fa fa-edit"></i></a>';
            @endif

            @if(Auth::user()->can('Product Add destroy'))

                <form id="" method="post" action="{{route('product.destroy',$d->id)}}">
                @csrf
                @method('DELETE')
